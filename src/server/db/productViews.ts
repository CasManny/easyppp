...
export const getProductViewCount = (userId: string, startDate: Date) => {
  const cacheFn = dbCache(getProductViewCountInternal, {
    tags: [getUserTag(userId, CACHE_TAG.productViews)],
  });
    
    return cacheFn(userId, startDate)
};

export const getProductViewCountInternal = async (
  userId: string,
  startDate: Date
) => {
